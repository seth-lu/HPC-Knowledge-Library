...
int main(int argc,char *argv[]) {
    ...
    string str_txt = "../data/data"+string(argv[1])+".txt";
    ofs.open(str_txt, ios::trunc);  //把这个流和文件关联起来
    ofs << "输出时间和GFLOPS" << endl;
    ofs << "tensor1d-times:" << endl;
    for(int i=0;i<times1d.size();++i){
        ofs << times1d[i] << endl;
    }
    ofs << "tensor1d-gflogs:" << endl;
    for(int i=0;i<gflops1d.size();++i){
        ofs << gflops1d[i] << endl;
    }
    ofs << "tensor4d-times:" << endl;
    for(int i=0;i<times4d.size();++i){
        ofs << times4d[i] << endl;
    }
    ofs << "tensor4d-gflogs:" << endl;
    for(int i=0;i<gflops4d.size();++i){
        ofs << gflops4d[i] << endl;
    }
    ofs << "wetensor-times:" << endl;
    for(int i=0;i<times4d.size();++i){
        ofs << timesWetensor[i] << endl;
    }
    ofs << "wetensor-gflogs:" << endl;
    for(int i=0;i<gflops4d.size();++i){
        ofs << gflopsWetensor[i] << endl;
    }
    ofs.close();
    ...
    string str_fig = "../figure/times"+string(argv[1])+".png";
    plt::save(str_fig);
    ...
    str_fig = "../figure/gflogs"+string(argv[1])+".png";
    plt::save(str_fig);
