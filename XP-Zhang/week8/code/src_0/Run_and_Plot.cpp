...
void directConvolution_tensor4d(const Tensor4D<double>& A,const Tensor4D<double>& B,Tensor4D<double>& C,int64_t s){
    for (int64_t i = 0; i < C.num_batch(); ++i){
        for (int64_t j = 0; j < C.num_channel(); ++j){
			for (int64_t m = 0; m < C.num_height(); ++m){
                for (int64_t n = 0; n < C.num_width(); ++n){
...
Tensor4D<double> directConvolution_tensor4d_t(const Tensor4D<double>& A,const Tensor4D<double>& B,int64_t s){
    Tensor4D<double> C(A.num_batch(),B.num_batch(), (A.num_height() - B.num_height()) / s + 1, (A.num_width() - B.num_width()) / s + 1);
...
int main() {
    ...
    for(int i = 0; i < MAXSIZE; ++i) {
        ...
        T.start();
        // C =  directConvolution_tensor1d_t(A,B,strides[i]);
        // C = A.convolutionWithStride(B,strides[i]);
        directConvolution_tensor(A,B,C,strides[i]);
        // A.convolutionWithStride_t(B,C,strides[i]);
        T.stop();
        elapsed = T.elapsed();
        ...
        T.start();
        // O = directConvolution_tensor4d_t(M,N,strides[i]);
        // O = M.convolutionWithStride(N,strides[i]);
        // M.convolutionWithStride_t(N,O,strides[i]);
